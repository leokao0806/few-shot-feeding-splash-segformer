...
class DenseConvModule(nn.Module):
    def __init__(self, c1, c2, total_subclass):
        super().__init__()
        self.conv = nn.Conv2d(c1, total_subclass, 1, bias=False)
        self.conv2 = nn.Conv2d(total_subclass, c2, 1, bias=False)
        self.activate = nn.ReLU(True)
...
class SegFormerHead(nn.Module):
    def __init__(self, dims: list, embed_dim: int = 256, num_classes: int = 19):
        ...
        self.dense_pred = DenseConvModule(embed_dim, num_classes, total_subclass=64)
    ...
    def forward(self, features: Tuple[Tensor, Tensor, Tensor, Tensor]) -> Tensor:
        ...
        seg = self.dense_pred(self.dropout(seg))
        return seg