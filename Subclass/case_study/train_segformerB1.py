...
from val_segformerB1 import evaluate  # 載入對應的驗證檔案
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='configs/ripple_B1.yaml')
    parser.add_argument('--k', type=int, default=0)
    args = parser.parse_args()

    with open(args.cfg) as f: config = yaml.load(f, Loader=yaml.SafeLoader)
    fix_seeds(3402)
    save_dir = Path(config['SAVE_DIR']) / f"train_segformer_b1_{args.k}"
